...
export default function Settings() {
  ...
  const handleImageChange = (e) => {
    ...
    reader.onloadend = () => {
      ...
      localStorage.setItem("employeeProfileImage", reader.result);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="pt-0 flex flex-col md:flex-row md:items-center justify-between pb-2">
        <div>
          <h1 className="text-xl font-semibold text-[#0D2426]">Settings</h1>
          <p className="text-xs text-[#6D8B8C]">Manage your account preferences</p>
        </div>
      </div>
            ...
            <div className="relative">
              <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-full bg-[#235857] overflow-hidden flex items-center justify-center text-white text-3xl font-black border-4 border-white shadow-md">
                {profileImage ? (
                  ...
                  user?.name?.split(' ').map(n => n[0]).join('').toUpperCase() || "EU"
                )}
              </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-blue-700 mt-1">
                  Role: <span className="font-semibold">{user?.role || "Employee"}</span>
                  {" · "}
      ...
      <section className="bg-white rounded-3xl border-l-4 border-[#1F6F68] shadow-sm overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          {[
            ...
            { key: "reminder", label: "Task Reminders", desc: "Get reminders before task deadlines" }
          ].map(({ key, label, desc }) => (
            <div
              key={key}
